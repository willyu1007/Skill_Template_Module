...
**Note**: Provider stubs (for `.codex/skills/`, `.claude/skills/`, etc.) should be generated using `node .ai/scripts/sync-skills.cjs`, not the land-skills script.

## When to use
Use land-skills-into-repo when:
...
Do **not** use land-skills-into-repo when:
...
The script always produces a plan/report to stdout; the script can also emit JSON via `--json-report`.
