...
This skill **does not** change `docs/context/observability/*` contracts. It only validates and syncs module slices.
...
2. If contracts are missing, initialize observability first:
   - `node .ai/skills/features/observability/scripts/obsctl.mjs init`
3. If new metrics/log-fields are needed, update the contracts via the repo-level workflow:
   - `node .ai/skills/features/observability/scripts/obsctl.mjs add-metric ...`
   - `node .ai/skills/features/observability/scripts/obsctl.mjs add-log-field ...`
...
Rules:
- **metrics.owns**: Metrics this module is responsible for emitting (must exist in metrics-registry.json)
- **metrics.uses**: Metrics this module depends on (emitted by other modules)
- **logs.owns**: Custom log fields this module defines (must exist in logs-schema.json)
- **logs.requires**: Log fields this module depends on (e.g., `trace_id`, `service`)
...
- MUST NOT modify observability contracts (`docs/context/observability/*`) in this skill.
