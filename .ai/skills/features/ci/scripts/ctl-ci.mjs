...
function upsertGitlabManagedBlock(filePath, beginMarker, endMarker, blockBody, dryRun) {
  ...
  const legacyBeginMarker = '# --- BEGIN DELIVERY (generated by cictl) ---';
  const legacyEndMarker = '# --- END DELIVERY (generated by cictl) ---';
  const hasDeliveryBlock =
    (raw.includes(beginMarker) && raw.includes(endMarker)) ||
    (raw.includes(legacyBeginMarker) && raw.includes(legacyEndMarker));
...
function cmdVerify(repoRoot) {
  ...
  if (!fs.existsSync(getCiDir(repoRoot))) {
    errors.push('ci/ directory not found. Run: node .ai/skills/features/ci/scripts/ctl-ci.mjs init');
  }
  ...
  if (config.provider === 'github') {
    ...
    if (!fs.existsSync(workflowDir)) {
      warnings.push('.github/workflows/ directory not found. Run: node .ai/skills/features/ci/scripts/ctl-ci.mjs init --provider github');
    } else {
