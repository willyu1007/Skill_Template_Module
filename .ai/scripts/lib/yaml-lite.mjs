 ...
 *
 * Provides simple YAML parsing for common patterns used in governance/meta files.
 * NOT a full YAML parser - only handles flat key-value pairs and simple lists.
 *
 * Usage:
 *   import { parseTopLevelVersion, parseSimpleList, parseSimpleMap } from './lib/yaml-lite.mjs';
 */
...
export function parseTopLevelVersion(raw) {
  const m = String(raw || '').match(/^\s*version\s*:\s*([0-9]+)\s*$/m);
 ...
 *
 * @param {string} raw - YAML content
 * @param {string} sectionKey - Section key to start parsing after (e.g., 'keywords')
 * @returns {string[]}
 */
