...
Examples:
  ...
  node .ai/scripts/ctl-project-state.mjs map --task T-001 --feature F-002 --apply
`.trim();
...
function cmdLint({ repoRoot, projectSlug, strict }) {
  ...
  if (!registry) {
    warnings.push(
      `Project hub is not initialized for project "${projectSlug}". Run: node .ai/scripts/ctl-project-state.mjs init --project ${projectSlug}`
    );
...
function cmdSync({ repoRoot, projectSlug, dryRun, apply, initIfMissing, changelog }) {
  ...
  if (!hubMissing) {
    ...
    if (!initIfMissing) {
      errors.push(
        `Project hub missing for "${projectSlug}". Run: node .ai/scripts/ctl-project-state.mjs init --project ${projectSlug}`
      );
      return { ok: false, errors, warnings, actions };
    }
