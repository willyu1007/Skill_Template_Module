...
function cmdVerify(repoRoot) {
  ...
  if (!fs.existsSync(statePath)) {
    errors.push('Project state file does not exist. Run: node .ai/scripts/ctl-project-state.mjs init');
  } else {
    const state = loadState(repoRoot);
    ...
    if (!state.context) {
      ...
      if (state.context.enabled !== true) {
        infos.push(
          'state.context.enabled is false (template default). ' +
          'Enable when Context Awareness is configured.'
        );
      }
    }
  }

  // Check project governance hub initialization
  const govHubDir = path.join(getProjectDir(repoRoot), 'main');
  const registryPath = path.join(govHubDir, 'registry.yaml');
  if (!fs.existsSync(registryPath)) {
    infos.push(
      'Project governance hub is not initialized (no .ai/project/main/registry.yaml). ' +
      'Run when needed: node .ai/scripts/ctl-project-governance.mjs init --project main'
    );
  }
